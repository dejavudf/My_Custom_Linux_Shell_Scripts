...
do
	clear
	VAR_VM_NAME=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "VM Name -> character only and size 4 (min) to 20 (max)" --inputbox 'VM Name:' 0 0 3>&2 2>&1 1>&3)
	if [ $? == 1 ]
	then
		exit
	elif [ $? == 255 ]
...
do
        clear
        VAR_VM_RAM=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "RAM Size -> 512Mb (min) to 8Gb (max)" --menu "RAM Size:" 0 0 5 "1" ": 512Mb" "2" ": 1Gb" "3" ": 2GB" "4" ": 4Gb" "5" ": 8Gb" 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
	then
		exit
	elif [ $? == 255 ]
...
do
        clear
        VAR_CPU=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "CPU Number -> 1 (min) to 6 (max)" --menu "CPU number:" 0 0 4 "1" ": 1 x CPU" "2" ": 2 x CPU" "3" ": 4 x CPU" "4" ": 6 x CPU" 3>&2 2>&1 1>&3)
	if [ $? == 1 ]
        then
                exit
	elif [ $? == 255 ]
...
do
        clear
        VAR_MACHINE_TYPE=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Machine Type -> old, new, isa or none" --menu "Machine Type:" 0 0 4 "1" ": Old PC i440FX" "2" ": New PC Q35" "3" ": Very Old PC ISA" "4" ": None" 3>&2 2>&1 1>&3)
	if [ $? == 1 ]
        then
                exit
	elif [ $? == 255 ]
...
do
	...
	clear
	VAR_OS_TYPE=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "OS Type and Variant -> Short ID" --inputbox 'OS Type and Variant - Short ID:' 0 0 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
                exit
        elif [ $? == 255 ]
	then
		...
		if [ -z $VAR_OS_TYPE ]
			...
			then
				cat ./tmp.tmp | awk '{print $1}' | grep -i "$VAR_OS_TYPE"
				if [ $? == 0 ]
				then
...
do
        clear
        VAR_GRAPH=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Grahp Mode -> vnc, console or none" --menu "Graph Mode:" 0 0 3 "1" ": VNC" "2" ": Console" "3" ": None" 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
		exit
        elif [ $? = 255 ]
...
do
        clear
        VAR_BOOT=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Boot Disk -> hd, cd, none" --menu "Boot Disk:" 0 0 3 "1" ": HD" "2" ": CD" "3" ": None" 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
                exit
        elif [ $? = 255 ]
...
do
        clear
        VAR_DISK_BUS=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Disk Bus -> ide, sata, scsi or virtio" --menu "Disk Bus:" 0 0 4 "1" ": IDE" "2" ": SATA" "3" ": SCSI" "4" ": Virtio" 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
                exit
        elif [ $? = 255 ]
...
do
        clear
        VAR_NET_MODEL=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "NIC Model (bridge mode): e1000, rtl8139 or virtio" --menu "Boot Disk:" 0 0 3 "1" ": e1000" "2" ": rtl8139" "3" ": virtio" 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
                exit
        elif [ $? = 255 ]
