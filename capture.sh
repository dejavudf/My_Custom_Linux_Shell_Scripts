...
then
	...
	then
		sudo tcpdump -C $var_size -W 1 -w "$var_dt""_Int_""$var_interface""_Target_""$var_target_net""_""CIDR""$var_target_subnet""_Protocol_""$var_protocol""_Port_""$var_port.pcap" -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet and proto \\$var_protocol and port $var_port
	elif [  "$var_save" == "y" ] && [ "$var_background" == "y" ]
	then
		sudo nohup tcpdump -C $var_size -W 1 -w "$var_dt""_Int_""$var_interface""_Target_""$var_target_net""_""CIDR""$var_target_subnet""_Protocol_""$var_protocol""_Port_""$var_port.pcap" -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet and proto \\$var_protocol and port $var_port &
	else
		sudo tcpdump -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet and proto \\$var_protocol and port $var_port
	fi
...
then
	...
	if [  "$var_save" == "y" ] && [ "$var_background" == "n" ]
        then
		sudo tcpdump -C $var_size -W 1 -w "$var_dt""_Int_""$var_interface""_Target_""$var_target_net""_""CIDR""$var_target_subnet""_Protocol_""$var_protocol""_Port_""$var_port.pcap" -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet
        elif [  "$var_save" == "y" ] && [ "$var_background" == "y" ]
        then
		sudo nohup tcpdump -C $var_size -W 1 -w "$var_dt""_Int_""$var_interface""_Target_""$var_target_net""_""CIDR""$var_target_subnet""_Protocol_""$var_protocol""_Port_""$var_port.pcap" -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet &
        else
		sudo tcpdump -ni $var_interface -Z $USER net $var_target_net/$var_target_subnet
        fi
fi
