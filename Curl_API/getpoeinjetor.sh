...
echo "NOME_AP;MAC_AP;PORTA_AP;SWITCH_IP;SWITCH_NOME" > ./lista.txt
...
do
	...
	cat $VAR_FILE | grep -i ge1 | while read -r VAR_VIZINHO
	do
		...
		then
			echo "$VAR_AP_NOME;$VAR_AP_MAC;$VAR_AP_PORTA_UPLINK;$VAR_SWITCH_IP;$VAR_SWITCH_NOME" >> ./lista.txt
		else
			continue
		fi
	done
