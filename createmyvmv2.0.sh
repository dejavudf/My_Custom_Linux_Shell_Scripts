...
do
	clear
	VAR_VM_NAME=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "VM Name -> character only and size 4 (min) to 20 (max)" --inputbox 'VM Name:' \
	0 0 3>&2 2>&1 1>&3)
	if [ $? == 1 ]
	then
		FUNC_MAIN_MENU
...
do
        clear
        VAR_VM_RAM=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "RAM Size -> 512Mb (min) to 8Gb (max)" --menu "RAM Size:" \
...
do
        clear
        VAR_CPU=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "CPU Number -> 1 (min) to 6 (max)" --menu "CPU number:" \
...
do
        clear
        VAR_MACHINE_TYPE=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Machine Type -> old, new, isa or none" --menu "Machine Type:" \
...
do
	...
	clear
	VAR_OS_TYPE=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "OS Type and Variant -> Short ID" --inputbox 'OS Type and Variant - Short ID:' \
	0 0 3>&2 2>&1 1>&3)
        if [ $? == 1 ]
        then
                FUNC_MAIN_MENU
...
do
        clear
        VAR_GRAPH=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Grahp Mode -> vnc, console or none" --menu "Graph Mode:" \
...
do
        clear
        VAR_BOOT=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Boot Disk -> hd, cd, none" --menu "Boot Disk:" \
...
do
        clear
        VAR_DISK_BUS=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "Disk Bus -> ide, sata, scsi or virtio" --menu "Disk Bus:" \
...
do
        clear
        VAR_NET_MODEL=$(whiptail --clear $VAR_BT_AQ --title "$VAR_T" --backtitle "NIC Model (bridge mode): e1000, rtl8139 or virtio" --menu "Boot Disk:" \
