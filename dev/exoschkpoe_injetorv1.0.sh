...
then
	...
	do
		if ../random -p $VAR_RANDOM ssh -o $VAR_KEY -o $VAR_ACE -o $VAR_HOST -o $VAR_STRICT -o $VAR_ALGO $VAR_USER@$VAR_IP "dis cli pag perm"
		then
			if ../random -p $VAR_RANDOM ssh -o $VAR_KEY -o $VAR_ACE -o $VAR_HOST -o $VAR_STRICT -o $VAR_ALGO $VAR_USER@$VAR_IP "show system | include SysName" > $VAR_IP"_sysname".tmp
			then
	        		if ../random -p $VAR_RANDOM ssh -o $VAR_KEY -o $VAR_ACE -o $VAR_HOST -o $VAR_STRICT -o $VAR_ALGO $VAR_USER@$VAR_IP "show lldp neighbors" > $VAR_IP"_lldp".tmp
	                	then
					if ../random -p $VAR_RANDOM ssh -o $VAR_KEY -o $VAR_ACE -o $VAR_HOST -o $VAR_STRICT -o $VAR_ALGO $VAR_USER@$VAR_IP "show inline-power stats ports" > $VAR_IP"_poe".tmp
					then
						...
						grep '[0-9]' ./$VAR_IP"_poe".tmp | awk '{print $1,$2}' > ./$VAR_IP.poe
							...
							VAR_NAME=$(echo "$VAR_LLDP" | awk '{print $4}')
								...
								then
									...
									echo $VAR_SYSNAME";"$VAR_IP";"$VAR_PORT";"$VAR_ID";"$VAR_PORT_ID";"$VAR_NAME >> $VAR_DIR/$VAR_DT"_"injetor_exos.csv
								fi
							fi
		        		        done < ./$VAR_IP.lldp
					else
						if ! cat ./failure_no_poe.log | grep -i $VAR_IP
						then
							echo "$VAR_IP - No POE Support" >> failure_no_poe.log
						fi
					fi
				else
